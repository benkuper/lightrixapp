...
class Node {
  ...
  Socket tcp; //for upload firmware
    ...
    if(isUploading)
    {
      if(msg == "upload ok") 
      {
         Fluttertoast.showToast(
          msg: "["+name+"] Upload complete", backgroundColor: Colors.green[500], textColor:Colors.green[100]);
          setUploading(false);
      }
      else Fluttertoast.showToast(msg: "["+name+"] "+lastMessage);
    }
  }
  ...
  void uploadFirmware(File f) async {
    ...
    {
        Fluttertoast.showToast(
          msg: "["+name+"] Node is not connected, not uploading.", backgroundColor: Colors.orange[500], textColor:Colors.orange[100]);
          setUploading(false);
    }
    ...
    Socket.connect(ip, 8888).then(socketConnected).catchError((error) {
      toastError("Error connecting to node : " + error.toString());
      tcp?.close();
    });
  }

  void socketConnected(Socket socket) async
  {
    tcp = socket;
  ...
  void toastError(msg) {
    Fluttertoast.showToast(
        msg:"["+name+"] "+ msg, backgroundColor: Colors.red, textColor: Colors.red[100]);
  }
